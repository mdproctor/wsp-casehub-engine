...
**Persistence direction (D1):** In-memory implementations now. All stores are case-scoped and low-mutation (human-timescale operator configuration). When durable implementations arrive, they follow the new platform direction: database-backed (likely CaseContext snapshot via #1166), EventLog for audit only. The EventLog writes from the #1132 spec (`GATE_PENDING`, `DENY_PATTERN_ADDED`, etc.) remain correct as audit trail — not as the durability mechanism. EventLog-replay-based recovery has known data-loss bugs (#1151, #1152) and is being superseded by #1150.

**SPI placement (D2):** SPIs in `common-core/spi` alongside `CaseInstanceRepository` and `EventLogRepository`. These are engine-internal coordination stores, not consumer-facing SPIs. The `api/spi/improvement` package is for SPIs external consumers implement (SummarizationProvider, EscalationProvider).
